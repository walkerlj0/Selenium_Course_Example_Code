...
This can be done to a specific element, or generically with Selenium's Action Builder ([link](https://selenium-python.readthedocs.io/api.html#module-selenium.webdriver.common.action_chains)). Either approach will send a key press. The latter will send it to the element that's currently in focus in the browser (so you don't have to specify a locator along with it), whereas the prior approach will send the key press directly to the element found.
...
After visiting the page we find the input element on the page and send the space key to it (e.g., `.send_keys(Keys.SPACE`). Then we grab the resulting text (e.g., `driver.find_element_by_id('result').text`) and assert that it says what we expect (e.g., `'You entered: SPACE'`).
...
When we save this file and run it (e.g. `python3 keyboard_keys.py` from the command-line) here is what will happen:

...
By using high quality locators we're already well ahead of the pack, but there are still some persnickity issues to deal with. Most notably... timing. This is especially true when working with dynamic, JavaScript heavy pages (which is more the rule than the exception in a majority of web applications you'll deal with).

But there is a simple approach that makes up the bedrock of reliable and resilient Selenium tests -- and that's how you wait and interact with elements. Gone are the days of waiting for the page to finish loading, or hard-coding sleeps, or doing a blanket wait time (a.k.a. an implicit wait). Nay. Now are the wonder years of waiting for an expected outcome to occur (e.g., waiting for an element to appear) for a specified amount of time.
...
Since there are two examples to choose from, we'll want to a simple way to load either one. So we created `loadExample`, which accepts a String of the example number we want to visit as an argument.

And similar to our login page object, we have a display check for the finish text (e.g., `finishTextPresent()`). This check is slightly different though. It has a second argument (an integer value of `10`). This second argument is how we'll specify how long we'd like Selenium to check to see if an element is displayed before giving up.

Let's update our base page object to enable explicit waits; modifying `isDisplayed` to use them and adding a second parameter for the timout interval (in seconds).
...
This method accepts two arguments -- the condition we want to wait for (e.g., `ExpectedCondition<WebElement>`) and the amount of time we want to wait for (`Integer timeout`). If a `null` value is specified, then we set the wait time to `5` seconds (e.g., `timeout = timeout != null ? timeout : 5;`).

In `isDisplayed` we add a second parameter for the timeout and swap out the previous Selenium action to use the new `waitFor` method along with an `ExpectedCondition` to check for the visibility of an element. This is the same thing as our previous display check, but it uses a different Selenium API function that will work with the explicit waits function. You can see a full list of Selenium's `ExpectedConditions` [here](http://selenium.googlecode.com/git/docs/api/java/org/openqa/selenium/support/ui/ExpectedConditions.html).

The second parameter is optional when calling `isDisplayed`. If a timeout is specified, it will get passed to the `waitFor` method`. If nothing is specified, `null` will get passed instead (which will default the timeout to `5` seconds). So we can call this method in our page objects without specifying a timeout (e.g., `isDisplayed(locator)`) or with a timeout (e.g., `isDisplayed(locator, 10)`).

## More On Explicit Waits

It's important to set a _reasonably sized_ default timeout for the explicit wait method. But you want to be careful not to make it too high. Otherwise you can run into a lot of the same timing issues you get from an implicit wait. But set it too low and your tests will be brittle, forcing you to run down trivial and transient issues.

In our page object when we're using `isDisplayed(finishText, 10)` we are telling Selenium to see if the finish text is visible on the page. It will keep trying until either the element is displayed or reaches ten seconds -- whichever comes first. If ten seconds is reached, Selenium will throw a timeout exception. We could catch the exception and return false just like we did with `NoSuchElementException`, but this would make the test less informative, so we'll let the exception through.
...
If you're thinking about mixing explicit waits with an implicit wait -- don't do it. If you use both together, you're going to run into issues later due to inconsistent implementations of implicit wait across the different browser drivers. Long story short, you'll see odd test behavior. You can read more about the specifics [here](http://stackoverflow.com/questions/15164742/combining-implicit-wait-and-explicit-wait-together-results-in-unexpected-wait-ti#answer-15174978).

### Part 2: Write A Test To Use The New Page Object

Now that we have our new page object and an updated base page, it's time to add our test.
...
The contents of this file are similar to `TestLogin` with regards to the imported libraries and the `setUp`/`tearDown` methods.
...
[The second dynamic loading example](http://the-internet.herokuapp.com/dynamic_loading/2) is laid out similarly to the last one, but it renders the final text __after__ the progress bar completes (whereas the previous example had the text on the page but it was hidden).

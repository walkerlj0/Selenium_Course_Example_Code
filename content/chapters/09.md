...
Since JavaScript is asynchronous we can't rely on something like a `try`/`catch` to handle our Selenium exceptions. Instead we'll need to leverage the promise handler that's built into the Selenium bindings to manually take control of the flow of the promise when doing an `isDisplayed` lookup.

First we require the promise class and store it in a `Promise` variable. Rather than declare `var` for it we can leverage a short-hand by using a comma and specifying it on a new line, which tacks it onto the `var` declaration for `driver`.

Next we call `Promise.defer();` in the `isDisplayed` method (storing it in a local variable). We then update `this.find(locator).isDisplayed()` to manually trigger a promise with a variable that represents it. If no exception is thrown then we want to return the promise (a.k.a. fulfill the promise) which is done with `defer.fulfill(isDisplayed)`. What comes next is the error callback (e.g., `}, function(error) {`).

When an exception is thrown this is where we'll handle it. We have access to the error that was thrown, so we can check its name and specify the return response based on it. So when the error name is `NoSuchElementError` (e.g., `if (error.name === 'NoSuchElementError')`) we can fulfill the promise with `false` instead throwing the exception (e.g., `defer.fulfill(false);`). This ensures that we only return `false` on this specific exception. We want all other exceptions to get thrown normally (which we handle in the `else` case of our conditional by calling `defer.reject(error);`). At the end of the method we make sure to return the promise (e.g., `return defer.promise;`). This makes sure that the caller (e.g., any page object that uses the `isDisplayed` method) gets a promise that it can use.

...
To create an instance of Selenium we call `webdriver.Firefox()` and store it in a variable. Since the name of the method is `driver`, we refer to this variable as `driver_`. This variable gets returned at the end of the method which will get used in our test (more on that soon).

...
If you're running on a Mac and you want to use [Homebrew](http://brew.sh/), then be sure to check out [this write-up from Treehouse](http://blog.teamtreehouse.com/install-node-js-npm-mac). And if you end up using WebStorm, check out the documentation on using it to work with Mocha ([link](https://www.jetbrains.com/help/webstorm/2016.2/running-mocha-unit-tests.html?search=mocha)).

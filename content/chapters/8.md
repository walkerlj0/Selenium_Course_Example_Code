...
And with this approach, we not only get the benefit of controlled chaos, we also get the benefits of reusable functionality across our suite of tests and more readable tests.
...
We'll need to create a new file for the page object. Let's create one called `Login.java` in the `pageobjects` package in our `src/tests/java` directory.
...
At the top of the file we specify the package where it lives and import the requisite libraries. We then declare the class (e.g., `public class Login`), specify our fields (for the Selenium instance and the page's locators), and wire up three methods.
...
The second method (e.g., `public void with(String username, String password)`) is the core functionality of the login page. It's responsible for filling in the login form and submitting it. By accepting strings parameters for username and password we're able to make the functionality here dynamic and reusable for additional tests.
...
Now the test is more concise and readable. And if you save this and run it (e.g., `mvn clean test` from the command-line), it will run and pass just like before.

### Part 2: Write Another Test

Creating a page object may feel like more work than what we started with initially. But it's well worth the effort since we're in a much sturdier position (remember: controlled chaos) and able easily write follow-on tests.

Let's add another test for a failed login to demonstrate.

First, let's take a look at the markup that gets rendered when we provide incorrect credentials:
...
This markup is similar to our successful flash message, so let's mimic the behavior we used in our page object to create another method to help in our assertion.
...
After importing the assertion library we put it to use in our constructor (after the Selenium action that visits the login page). With it we're checking to see that the login form is displayed. If it is, the tests using this page object will proceed. If not, the test will fail and provide an output message stating that the login form wasn't present.

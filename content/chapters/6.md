...
If we run this (e.g., `mvn clean test` from the command-line), it will run and pass. But there's one thing missing -- an assertion. In order to find an element to make an assertion against, we need to see what the markup is after submitting the login form.
...
There are a couple of elements we can ues for our assertions in this markup. There's the flash message class (most appealing), the logout button (appealing), or the copy from the `h2` or the flash message (least appealing).
...
Now when we run this test (`mvn clean test` from the command-line) it will pass just like before, but now there is an assertion which will catch a failure if something is amiss.

...
We haven't written a Grunt task for `--ci=on` yet. So let's hop over to `Gruntfile.js` and do that before we proceed.
...
We added a new `shell` execution block for running tests on CI (e.g., `runTestsOnCi`). In it we're specifying some additional command-line options for outputting XML that Jenkins can consume. Specifically `-R xunit > test-result-'+testFile+'.xml'`. `-R` is used to specify a different report, in this case `xunit`. And to output the results to a file we use `>` and specify a file name. Since there are concurrent threads running we need each result file to be uniquely named. And since no two files will ever have the same name, we use the filename.
...
With `grunt.option('ci')` we're checking to see if a value is passed in with `--ci` and storing the result in a `ci` variable. If something is passed in then we use it. If not then we set it to `'off'`. We then use the `ci` variable in a conditional when registering our tasks. If `ci` is set to `'on'` then we use the `runTestsOnCI` `shell` executor, otherwise, we use our regular `runTest` `shell` executor.
...
In the `quit` method of our Driver Factory we make it so we throw an exception with the Sauce Labs job URL when there's a test failure.

...
To use Sauce Connect, you need to download and run it. There's a copy for each operating system -- get yours [here](https://docs.saucelabs.com/reference/sauce-connect/) and run it from the command-line. In the context of our existing test code, let's download and store Sauce Connect in our `vendor` directory.

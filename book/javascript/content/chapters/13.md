...
Selenium Grid lets you distribute test execution across several machines and you connect to it with Selenium `RemoteWebDriver`. You tell the Grid which browser and OS you want your test to run on through the use of Selenium's exported class object [`selenium-webdriver.Capabilities` (i.e. `DesiredCapabilities`)](https://www.selenium.dev/selenium/docs/api/javascript/module/selenium-webdriver/index_exports_Capabilities.html), and its various subclasses for specific browser options (`Capabilities.chrome()`, `Capabilities.firefox()`, etc.)
...
__NOTE: In JavaScript, functions or methods prefixed with a `_` are intended to be private.__

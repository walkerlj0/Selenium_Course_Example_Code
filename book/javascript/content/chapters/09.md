...
```javascript
    assert(await !login.successMessagePresent(), 'Success message was displayed instead of failure message')
```
...
We could use a visual check here as well.
...
// ...
  async load() {
    await this.driver.get('http://the-internet.herokuapp.com/login')
    if (await !this.findElement(LOGIN_FORM).isDisplayed())

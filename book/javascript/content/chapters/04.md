...
+ Scope
+ Types of Objects (Strings, Integers, Collections, Booleans, etc.)
...
Throughout our test code we'll want to group actions together for easy reuse. We can do this by placing them into either functions or methods.

Functions can be specified and called on their own, whereas methods are functions that are tied to a class (more on classes soon).

When it comes to naming, they follow the same rules as variables. However, function and method names tend to be a verb (since they denote some kind of an action to be performed), and the contents of them are wrapped in opening and closing brackets (e.g., `{}`).
...
Methods are specified as part of a class and don't require any special keywords.
...
As you can see, we can specify arguments we want to pass into a function or method when calling it (a.k.a. specifying a parameter).
...
Variables declared in classes will enable you to store and retrieve values in all methods within the class.

Variables declared outside of a class will be available throughout the file.
...
Variables that are fully capitalized and separated by underscores (e.g., `_`) are called constants. They are variables that won't change.

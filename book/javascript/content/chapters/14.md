...
In addition to the `baseUrl` and `browser` variables, we've added some more (e.g., `host`, `browserName`, `version`, `platform`, `sauceUsername`, and `sauceAccessKey`).

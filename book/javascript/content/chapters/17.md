...
```text
BROWSER=chrome BROWSER_VERSION=74 PLATFORM='Windows 10' mocha-parallel-tests --grep=@shallow -R xunit test/*Test.js
```
...
+ Add the name of the result file specified in the command -- `*.xml`
+ Click `Save`
...
There are numerous ways to go about this (e.g., e-mail, chat, text, co-located visual cues, etc). Thankfully there are numerous, freely available plugins that can help facilitate whichever method you want. You can find out more about Jenkins' plugins [here](https://wiki.jenkins-ci.org/display/JENKINS/Plugins).

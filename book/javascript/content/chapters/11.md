...
__NOTE: We have to wait for an element to be present before doing a display lookup because the display lookup requires the use of a *found* element (not a locator).__
...
In `beforeEach` we've omitted the Eyes configuration, since we won't be using visual validations in these tests.

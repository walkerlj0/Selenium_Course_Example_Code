...
When running tests with visual validations we'll need to specify a viewport size. Right now this value is hard-coded in our setup code. We can move it to our config file, and make it so the sensible defaults can be overridden at run-time if needbe.

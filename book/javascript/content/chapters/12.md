...
Now when running our tests, we can specify a different base URL or viewport size by providing some extra information at run-time (e.g., `BASE_URL=url HEIGHT=1234 WIDTH=567 mocha`). We're also in a better position now with our setup and teardown abstracted into a central location.

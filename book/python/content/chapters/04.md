...
Assertions are made against booleans and result in either a passing or failing test. In order to leverage assertions we will need to use a testing framework (e.g., [pytest](http://docs.pytest.org/en/latest/) or [unittest](https://docs.python.org/2/library/unittest.html). For the examples in this book we will be using `pytest` (version `2.9.2`).

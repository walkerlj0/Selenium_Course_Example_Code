...
In the `TearDown` method of our BaseTest we make it so we throw an exception with the Sauce Labs job URL when there's a test failure and ensure that `Driver.Quit()` is called regardless. This change will make the job URL show up in a relevant spot in the XML test report.
